...
/**
 * horwheel   参数:rm = removeEvent 移除滚动条横向滚动事件
 * var horwheel = require('horwheel');
 *        var wrapper = document.querySelector('dom节点');
 *        horwheel(wrapper);
 *
 */
var el_dom=null;
function horwheel(el,rm) {
    if (el === undefined) {
        return;
    }
	el_dom=el;
	if(rm=='removeEvent'){
		document[unbind](wheel,horizontalWheel_bd,false);
		return false;
	}else{
		document[bind](wheel,horizontalWheel_bd,false);
		return false;
	}
}


function horizontalWheel_bd(eve) {
	eve = eve || window.eve;
	if (eve.preventDefault) {
		eve.preventDefault();
	}
	horizontalWheel(eve, el_dom);
	return false;
}
